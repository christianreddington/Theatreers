...
namespace Theatreers.Admin.Utilities
{
    public class FunctionAuthorizeAttribute : FunctionInvocationFilterAttribute
    {
        ...
        public static ValidationPackage ExtractClaims(string jwt, JwtSecurityTokenHandler handler)
        {
            ...
            if ((validationPackage.ExpiresAt - currentTimestamp) > 0 && IsUserAuthorized(validationPackage.Permissions, "admin"))
            {
        ...
        public static bool IsUserAuthorized(string permissions, string microservice)
        {
            if (permissions.Contains($"{microservice}:owner") || permissions.Contains($"{microservice}:contributor"))
            {
                return true;
            }
